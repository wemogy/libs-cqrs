...
using Wemogy.CQRS.Extensions.AzureServiceBus.UnitTests.Testing.Commands.AlwaysFailingSession;
...
public class AzureServiceBusCommandSessionProcessorDeadLetterTests
{
    private const string QueueName = "unit-testing-queue-failing-sessions";
    private readonly ICommands _commands;
    private readonly IServiceProvider _serviceProvider;
    private readonly ServiceBusClient _serviceBusClient;

    public AzureServiceBusCommandSessionProcessorDeadLetterTests()
    {
        ...
        serviceCollection
            ...
            .ConfigureDelayedProcessing<AlwaysFailingSessionCommand>(builder =>
            {
                builder
                    .WithQueueName(QueueName)
                    .WithSessionSupport();
            })
            .AddDelayedSessionProcessor<AlwaysFailingSessionCommand>(maxDeliveryCount: 1);
    ...
    public async Task HandleMessageAsync_ShouldDeadLetterWithActualException_WhenMaxDeliveryCountReached()
    {
        ...
        var hostedService = _serviceProvider
            ...
            .OfType<IAzureServiceBusCommandProcessorHostedService<AlwaysFailingSessionCommand>>()
            .First();
        await hostedService.StartAsync(CancellationToken.None);
        await Task.Delay(TimeSpan.FromSeconds(5));

        // Act
        await _commands.ScheduleAsync(
            new AlwaysFailingSessionCommand(sessionId),
            new DelayOptions<AlwaysFailingSessionCommand>(TimeSpan.Zero, x => x.SessionId));
        ...
        dlqMessage.DeadLetterErrorDescription.Should().Be(AlwaysFailingSessionCommandHandler.ExceptionMessage);
